    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            Task.Run(async () =>
            {
				...
				var list = await PackageHelper.GetPackageAndDependencysAll("Azure.Identity", NuGetFramework.Parse("net48"), "1.3.0");
				await Console.Out.WriteLineAsync("corext.config: ");
				foreach (var item in list)
				{
					await Console.Out.WriteLineAsync(@$"<package id=""{item.Id}"" version=""{item.Version.OriginalVersion}"" />");
				}
                ...
                foreach (var item in list)
                {
                    ...
                    await Console.Out.WriteLineAsync(@$"<Reference Include=""{item.Id}"">
				    <HintPath>..\packages\{item.Id}.{item.Version.OriginalVersion}\lib\{item.Tfm.GetNetPath()}\{item.Id}.dll</HintPath>
				</Reference>");
                }
